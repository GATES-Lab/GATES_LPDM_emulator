...
| C2 | Implemented, awaiting commit | **Yes** (dynamic-edge runs only; retrain) | — | **Unflagged** (clear bug; `dynamic_earthdistance` is opt-in, off by default). `encoder.py` `SatelliteDynamicEncoder.forward`: `src_lat` now reads `features[:, latlon_indices[0], :]` (lat) instead of `[1]` (lon), so the per-edge haversine is `haversine(src_lat, src_lon, dst_lat, dst_lon)` as intended; the other three endpoints were already correct. **Hardening in the same change** (`training.py setup_dynamic_edges`): `latlon_indices` is now resolved by looking up each `latlon_tuple` in `[lat, lon]` order via a name→index map, so `latlon_indices[0]` is *always* lat and `[1]` lon regardless of `input_names` order — the old list-comprehension returned them in `input_names` order, which would silently re-break C2 if a config listed `lon_coords` before `lat_coords` (the `len==2` check couldn't catch it). Missing coords now raise a targeted error. Output-neutral for all current configs (they list `lat_coords` before `lon_coords`); affects only `dynamic_earthdistance` runs (e.g. `exp06`). |
| C3 | Committed | **Yes** (retrain) | `04727d0` | Behind `review_fixes.use_tensor_loader` flag (default False = xbatcher). New additive `make_tensor_dataloader` in `datasets.py` materialises the scaled inputs/fps into an in-memory `TensorDataset`; `DataLoader(shuffle=True)` then reshuffles batch **composition** every epoch natively (no per-epoch rebuild, `run_full_training` untouched). Train loader only; test stays on xbatcher; xbatcher path kept intact behind the else branch. Doubles as the review's P2 train-path groundwork. Local `torch.Generator` seed sidesteps C24 on this path. `resolve_tensor_loader_params` (next to the loader) cleans the reused xbatcher-style `dataloader_params` (drops worker/context keys + keys the loader sets explicitly; forces `num_workers=0`) — this both fixes a `shuffle` kwarg collision and, crucially, is **not silent**: `setup_GATES_dataloaders` prints requested-vs-used and **replaces** `parameters["dataloader"]["dataloader_params"]` with the used set so the saved `training_settings` reflect (and future re-runs use) the real config. Prints `[review_fixes] Using in-memory TENSOR train loader ...` when active. Verified vs xbatcher on synthetic data: identical labels/shapes/values with shuffle off; batch composition changes epoch-to-epoch with shuffle on. Part of the C1/C3 2×2 factorial. |
| C14 | Implemented, awaiting commit | **Yes** (attention runs only; retrain) | — | **Unflagged self-attention fix only** (`attention` is opt-in, off by default). Both encoder attention-mask sites now `fill_diagonal_(0)` instead of `(1)`: the mask is `1 - to_dense_adj(edges)` (0 = allowed, 1 = masked, `.bool()`-ed before `nn.MultiheadAttention`), and the k-ring includes the cell itself so the diagonal started at 0 (allowed) and was being flipped to 1 (masked) — so every mesh node attended to its neighbours but **not itself**. Setting the diagonal to 0 explicitly guarantees self-attention regardless of self-loops. Fixed in the live `SatelliteDynamicEncoder` (`encoder.py:718`) and the obsolete `SatelliteEncoder` (`encoder.py:319`) for consistency. **Scope:** self-attention only, per the review's Medium severity. The deeper design issue is NOT touched: `NodeSatelliteProcessorAttention` (`graph_net_block.py:815-877`) runs dense self-attention over all mesh nodes and **discards `edge_index`/`edge_attr`** for the node update, so turning `attention` on still throws away the wind/distance/release edge features. Principled follow-up = edge-featured attention (GATv2 / transformer-conv using `edge_attr` as a bias). Output-neutral unless `attention=true`. |
| Met chunking (P3-adjacent) | Committed | Yes (loading only) | `04727d0` | Removed the `{time: chunk_size}` coalesce-rechunk added in `31161bf` from `_cut_satellite_met_multi_delta`. On the shared store it forced each per-crop block to read whole `chunk_size` time-blocks (over-read) rather than the exact native `{time:1}` chunks, adding I/O instead of saving it (see the load-time investigation). The block crop now reads native `{time:1}` chunks again. Does not change model outputs — affects data-loading cost only. |
