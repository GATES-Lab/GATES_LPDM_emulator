...
class GraphSatelliteBackgroundPredictor(torch.nn.Module): #, PyTorchModelHubMixin
    """Model to predict a background concentration value for the whole grid, rather than a per-node output. This is an extension to GraphSatelliteForecaster, which produces a per-node output for the whole grid."""
    def __init__(
        self,
        lat_lons: list,
        whole_world: bool = False,
        ...
        Args:
            ...
            decoder_type: Type of decoder to use. Either "conv" for a decoder with a conv block and linear layer to produce num_classes outputs, or "dense" for a decoder with just a linear dense layer to produce num_classes outputs.
        """
        ...
        if decoder_type=="conv":
            self.decoder = SatelliteDecoderConvPredictor(
                lat_lons=lat_lons,
                h_grid=self.encoder.h3_grid,
                whole_world=whole_world,
                resolution=resolution,
            ...
            self.decoder = SatelliteDecoderPredictor(
                lat_lons=lat_lons,
                h_grid=self.encoder.h3_grid,
                whole_world=whole_world,
                resolution=resolution,
