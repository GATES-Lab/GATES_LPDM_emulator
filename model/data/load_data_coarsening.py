...
class _CleverTransform3XR:
    ...
    def __init__(self, parent):
        self.parent = parent
        self.transformers = {}
        self.vars_to_ignore = {"sin_time_year", "cos_time_year"} # Nawid - transformations to occur
    ...
    def transform(self):
        transformed_data = []
        variable_names = self.parent.inputs.coords["variable_name"].values # Nawid - get the variable names
        ...
        for name_tuple in variable_names:
            ...
            da = self.parent.inputs.sel(variable_name=name_tuple)
            flat_data = da.values.reshape(-1, 1) # Nawid - reshape the array
            ...
            if self.parent.mode == "train":
                ...
                self.transformers[name_tuple] = scaler # Nawid - transformer for the particular 
            else:
                scaler = self.transformers[name_tuple]

            scaled = scaler.transform(flat_data).reshape(da.shape) # Nawid - transforms the data
            scaled_da = xr.DataArray( # Nawid - get the transformed data into the format appropriate for the data
                scaled,
                dims=da.dims,
                coords=da.coords,
                name="transformed"
            )
            transformed_data.append(scaled_da) # Nawid -collagtes the transformed data
        ...
        self.parent.transform_parameters["clever_transform_3"] = {"transformers": self.transformers} # Nawid - parameters for the transformation
