#!/bin/bash
#SBATCH --job-name=is_SA_match_oracle_BRAZIL_with_landover
...
#SBATCH --gres=gpu:1
#SBATCH --time=6:00:00
#SBATCH --output=output_logs/%j_is_SA_match_oracle_BRAZIL_with_landcover.out

# Runs inside the apptainer container; no conda/module environment is used. Submit with:
#   sbatch launch_isambard_SA.sh
...
PARAM_FILE="${PARAM_FILE:-NEW_parameter_SA.json}"
