...
class LoadBaseSatelliteData:
    ...
    def load_meteorology(self, met_datadir=None, met_levels = [], met_variables= [], lazy_load=True, parallel=False, met_chunksize=8):
        """Load meteorology and select the met levels and variables if required.

        Args:
            met_datadir (str, optional): Directory for meteorology. Default directs
                to ACRG meteorology folder. If passing as an argument, the date will
                be automatically added, so the files should have format
                name_of_your_choice_yearmonth.nc (e.g. brazil_201601.nc) and you
            ...
            lazy_load (bool, optional): If True, does not load the met data into
                ...
                Defaults to True.
            parallel (bool, optional): <FILL IN>. Defaults to False.
            met_chunksize (int, optional): <FILL IN>. Defaults to 8.
    ...
    def _get_meteorology_file(self, met_datadir, lazy_load=True, parallel=False,met_levels=[], met_variables=[], met_chunksize=8):
        ...
        If ``met_datadir`` is None, uses default directory and file format. If
        ``met_datadir`` is passed, the date will be automatically added, so the files
        should have format example_name_yearmonth.nc (e.g. brazil_201601.nc) and you
        should pass ``met_datadir="/path/example_name_"``.

        Args:
            met_datadir (str or Path or None): Directory/prefix for the meteorology files.
            lazy_load (bool, optional): <FILL IN>. Defaults to True.
