...
sys.path.insert(1, "/user/work/yl18410/graphnet/graphnet_LPDM_emulator/")
...
path="/user/work/yl18410/new_graphnet/graph_weather/trained_site_models/"
...
def load_file(file_name, file_path):
    ...
    if not file_path:
       file_path ="/user/work/yl18410/new_graphnet/graph_weather/train_satellite_files/"
...
data = LoadSquareSiteData(**train_load_data)
test_data = LoadSquareSiteData(**test_load_data)
...
for epoch in range(302):
    ...
    if epoch % 5 == 0:
        ...
        for axis, fn in enumerate(image_plots):
            ...
            ax[1,axis].set_title(f"truth, \n sample {fn} ({str(image_dates[axis][:-10])})")
